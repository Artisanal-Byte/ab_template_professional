...
// Dropdown style options map to DropdownMenu `variant` prop.
const dropdownVariantOptions = [
  { label: 'Popover', value: 'popover' },
  { label: 'Inline', value: 'inline' },
];

// Side options reflect DropdownMenu `side` prop.
const sideOptions = [
...
// Align options reflect DropdownMenu `align` prop.
const alignOptions = [
...
// Offset options map to DropdownMenu `sideOffset` prop values.
const offsetOptions = [
  { label: '4px', value: '4' },
  { label: '6px', value: '6' },
  { label: '10px', value: '10' },
...
// Trigger width options map to DropdownMenu `width` prop.
const widthOptions = sharedWidthOptions;

// Menu width options map to DropdownMenu `menuWidth` prop.
const menuWidthOptions = [
  { label: 'Match trigger', value: 'auto' },
  { label: 'XS', value: 'xs' },
  { label: 'SM', value: 'sm' },
  { label: 'MD', value: 'md' },
  { label: 'LG', value: 'lg' },
...
// Trigger variant options map to DropdownMenu `triggerVariant` prop.
...
// Trigger size options map to DropdownMenu `triggerSize` prop.
const sizeOptions = [
...
// Disabled options map to trigger `disabled` attribute.
...
const triggerVariant = ref('secondary');
const triggerSize = ref('md');
const triggerDisabled = ref('false');
const triggerLabel = ref('Open menu');
const itemLabel = ref('');
const items = ref(['Item 1']);
const destructiveLabel = ref('Sign out');

const isDisabled = computed(() => triggerDisabled.value === 'true');
...
const buildDropdownAttrs = () =>
  buildAttrList([
    { when: !isInline.value && side.value !== 'bottom', attr: `side="${side.value}"` },
    { when: !isInline.value && align.value !== 'start', attr: `align="${align.value}"` },
    { when: !isInline.value && sideOffset.value !== '6', attr: `:side-offset="${sideOffset.value}"` },
    { when: width.value !== 'md', attr: `width="${width.value}"` },
    { when: !isInline.value && menuWidth.value !== 'auto', attr: `menu-width="${menuWidth.value}"` },
    { when: triggerVariant.value !== 'secondary', attr: `trigger-variant="${triggerVariant.value}"` },
    { when: triggerSize.value !== 'md', attr: `trigger-size="${triggerSize.value}"` },
    { when: isDisabled.value, attr: 'trigger-disabled' },
  ]);
...
const buildMenuLines = () => {
  const lines: string[] = [];
  const entries = menuItems.value;
  entries.forEach((item) => {
    lines.push(
      `    <button class="w-full rounded-md px-2 py-1 text-left hover:bg-secondary-hover">${item}</button>`,
    );
  });
  if (entries.length) {
    lines.push('    <div class="my-1 h-px bg-border"></div>');
  }
  lines.push(
    `    <button class="w-full rounded-md px-2 py-1 text-left text-error hover:bg-error-soft">${destructiveLabel.value || 'Sign out'}</button>`,
  );
  return lines.length
    ? lines.join('\n')
    : '    <button class="w-full rounded-md px-2 py-1 text-left hover:bg-secondary-hover">Menu item</button>';
};
...
<template>
  <section class="grid gap-6">
    <div class="grid gap-2">
      <h2 class="text-2xl font-semibold">Dropdown menu playground</h2>
      <p class="text-sm text-foreground-subtle">
        Configure placement, trigger styles, and copy the generated snippet.
      </p>
    </div>
        ...
        <div class="flex flex-col gap-3">
          ...
          <div class="flex flex-wrap gap-2">
            <div
              ...
              class="flex items-center gap-2 rounded-full border border-border-subtle bg-secondary-soft px-3 py-1 text-xs text-foreground"
            >
              <span>{{ item }}</span>
              <button
                type="button"
                class="rounded-full p-0.5 text-foreground-faint transition hover:text-foreground"
