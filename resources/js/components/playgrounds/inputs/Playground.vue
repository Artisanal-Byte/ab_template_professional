...
import { booleanOptions, sizeOptions, yesNoOptions } from '@/components/playgrounds/options';
import { buildAttrList } from '@/components/playgrounds/buildAttrList';

// Input size options reflect Input.vue `size` prop.
const sizes = sizeOptions;

// Input type options map to the native `type` attribute.
const types = [
...
// Required options map to the native `required` attribute.
const requiredOptions = yesNoOptions;
...
const buildInputAttrs = () =>
  buildAttrList([
    ...
    { when: type.value !== 'text', attr: `type="${type.value}"` },
